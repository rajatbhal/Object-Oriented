...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		//library that'll will help to make map interactive
	...
	private void addKey() {	
		...
		text("City Marker", 85, 95);
		text("Ocean Quake", 85, 115);
		text("Land Quake", 85, 135);
		
		text("Size ~ Magnitude",50, 155);
		text("Shallow", 85, 175);
		text("Intermediate", 85, 195);
		text("Deep", 85, 215);
		text("Past-Hour", 85, 235);
		
		fill(255,255,255);
		ellipse(60, 135, 10, 10);
		ellipse(60, 175, 10, 10);
...
//		text("Below 4.0", 75, 225);
	...
	private void printQuakes() 
	{
		...
		int earthquake_in_ocean = quakeMarkers.size();
		for(Marker cm: countryMarkers) {
			int number_of_quakes_on_land = 0;
			String name_of_country = cm.getProperty("name").toString();
			for(Marker qm: quakeMarkers) {
				EarthquakeMarker eqm = (EarthquakeMarker)qm;
				if(eqm.isOnLand()) {
					if(name_of_country.equals(eqm.getProperty("country"))) {
					number_of_quakes_on_land++;
					}
				}
			}
			
			if(number_of_quakes_on_land > 0) {
				earthquake_in_ocean -= number_of_quakes_on_land;
				System.out.println(name_of_country + ": " + number_of_quakes_on_land);
			}
		}
		
		System.out.println("OCEAN_QUAKE: " + earthquake_in_ocean);
