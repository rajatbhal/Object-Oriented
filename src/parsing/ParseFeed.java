...
public class ParseFeed {
	...
	public static List<ShapeFeature> parseRoutes(PApplet p, String fileName) {
		...
		for(String row : rows) {
			...
			if(!columns[2].equals("\\N")) route.putProperty("airport_code", columns[2]);
			if(!columns[4].equals("\\N")) route.putProperty("destination_code", columns[4]);
			// check that both airports on route have OpenFlights Identifier
			if(!columns[2].equals("\\N") && !columns[3].equals("\\N") && !columns[5].equals("\\N")){
